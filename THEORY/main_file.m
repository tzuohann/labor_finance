...
i_gamma = 1;
i_FC = 1;
[params,tech] = param(i_sigma,i_b,i_gamma,i_FC,0);

for i_1 = 1:length(params.ssigma_grid)
      for i_2 = 1:length(params.b_grid)
            for i_3 = 1:length(params.gamma_grid)
                  for i_4 = 1:length(params.fix_cost_grid)
                        
                        %Parameterization
                        WS = NaN;
                        [params,tech] = param(i_1,i_2,i_3,i_4,WS);
                        
                        %Display which parameters you are using
                        disp(['sigma = ',num2str(i_1)])
                        disp(['b = ',num2str(i_2)])
                        disp(['gamma = ',num2str(i_3)])
                        disp(['W = ',num2str(i_4)])
                        ...
                        FileName = [BaseName,'_sigma',num2str(i_1),...
                              '_b',num2str(i_2),'_gamma',num2str(i_3),'_W',num2str(i_4)]
                        save(FileName)
                  end
            end
      end
