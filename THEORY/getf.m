  ...
  switch whichCommitment
    case {'perfect'}
      for ip = 1:length(phi_vec)
        phi           = phi_vec(ip);
        if phi < phi_e
          f_point = (U_given - (1+BETA+BETA^2)*utilFunc(b,ssigma,1))...
            /(BETA*(E2_discrete + BETA*utilFunc(b,ssigma,1) ...
            - (1+BETA)*utilFunc(b,ssigma,1)));
        elseif phi >= phi_e
          f_point = (U_given - (1+BETA+BETA^2)*utilFunc(b,ssigma,1))...
            /(BETA*(E2_discrete + BETA*E3_discrete ...
            - (1+BETA)*utilFunc(b,ssigma,1)));
        end
      end
      f_discrete = f_discrete + f_point;
      f_discrete = f_discrete/length(phi_vec);
    case {'limited'}
      for ip = 1:length(phi_vec)
        phi           = phi_vec(ip);
        if phi < phi_lim
